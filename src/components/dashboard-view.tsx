    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-24">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-bold text-gray-900 flex items-center"><Store className="w-5 h-5 mr-2 text-[#4169E1]"/> My Listings</h3>
        </div>
        ...
        {loadingListings ? (
          ...
          <EmptyListings onAddListing={() => setIsCreatingListing(true)} />
        ) : (
          <div className="space-y-4">
            {listings.map((listing) => (
              <div key={listing.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col hover:shadow-md transition">
                <div className="flex p-4 gap-4">
                  <div className="w-24 h-24 rounded-xl bg-gray-100 relative overflow-hidden shrink-0 border border-gray-200">
                    <Image 
                      src={listing.images ? JSON.parse(listing.images)[0] : (listing.coverImage || 'https://placehold.co/400x400/eeeeee/999999?text=No+Image')} 
                      alt={listing.name} 
                      fill 
                      className="object-cover" 
                    />
                    {listing.isPremium && (
                      <div className="absolute top-0 right-0 bg-[#D4AF37] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-bl-lg z-10 flex items-center">
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                    <div>
                      <h4 className="font-bold text-gray-900 text-lg truncate">{listing.name}</h4>
                      <p className="text-sm text-gray-500 truncate">{listing.category} • {listing.city?.name}</p>
                    </div>
                    <div className="flex items-center gap-3 mt-2">
                      <div className="flex items-center text-gray-500 text-sm font-medium bg-gray-50 px-2 py-1 rounded-md">
                        <Eye className="w-4 h-4 mr-1" /> {listing.viewsCount}
                      </div>
                ...
                <div className="flex border-t border-gray-100 bg-gray-50/50">
                  <Button variant="ghost" className="flex-1 rounded-none text-gray-600 hover:text-[#4169E1] hover:bg-[#4169E1]/10 font-medium h-12">
                    <Pencil className="w-4 h-4 mr-2" /> Edit
                  </Button>
                  <div className="w-[1px] bg-gray-200" />
                  <Button variant="ghost" onClick={() => deleteListing(listing.id)} className="flex-1 rounded-none text-red-500 hover:text-red-600 hover:bg-red-50 font-medium h-12">
      ...
      <div>
        <div className="flex items-center justify-between mb-4 mt-8">
          <h3 className="text-xl font-bold text-gray-900 flex items-center"><ImageIcon className="w-5 h-5 mr-2 text-[#4169E1]"/> My Banners</h3>
        </div>
        ...
        {loadingBanners ? (
          ...
          <div className="text-center py-8 text-gray-500 font-medium bg-white rounded-2xl shadow-sm border border-gray-100">You don't have any active banners.</div>
        ) : (
          <div className="space-y-4">
            {banners.map((banner) => (
              <div key={banner.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden relative">
                ...
                <div className="p-3 bg-white flex justify-between items-center">
                  ...
                  {banner.linkUrl && (
                    <a href={banner.linkUrl} target="_blank" rel="noreferrer" className="text-sm font-medium text-[#4169E1] hover:underline flex items-center">
                      View Link <ArrowUpRight className="w-3 h-3 ml-1" />
                    </a>
                  )}
                </div>
              </div>
      ...
      <button 
        ...
        className="fixed bottom-24 right-6 w-16 h-16 rounded-full bg-gradient-to-r from-[#D4AF37] to-[#F5D76E] text-white shadow-[0_8px_30px_rgba(212,175,55,0.4)] flex items-center justify-center hover:scale-105 active:scale-95 transition-transform z-40"
      >
